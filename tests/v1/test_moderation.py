...
def test_trigger_moderation_same_post_twice(client, test_user, auth_token, test_post, db_session) -> None:
...
def test_vote_on_moderation_not_harmful(client, test_user, auth_token, test_post, db_session) -> None:
...
def test_moderation_flow_with_multiple_votes(client, test_user, other_user, auth_token, other_auth_token, test_post, db_session) -> None:
    ...
    # Other user votes harmful (corrected URL)
    response = client.post(
        "/api/v1/moderation/vote",
        params={"post_id": test_post.id, "is_harmful": True},
        headers=other_auth_token
    ...
    # Another test user votes not harmful
    response = client.post(
        "/api/v1/moderation/vote",
        params={"post_id": test_post.id, "is_harmful": False},
        headers=auth_token
    ...
    # Check moderation queue contains the post
    response = client.get("/api/v1/moderation/queue")
    assert response.status_code == status.HTTP_200_OK
    queue = response.json()
